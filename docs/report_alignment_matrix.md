...
| Preprocess and 298-feature contract | Report-aligned feature stack with canonical 298-length vector | src/dsp/preprocess.py, src/dsp/features.py, src/dsp/wavelet_features.py | tests/test_preprocess.py, tests/test_feature_shape.py, tests/test_teensy_dsp_parity.py | Implemented |
| Synthetic dataset generation | 38,500 balanced synthetic samples | ML teammate scope | docs/model_prd.md | ML-owned |
| M1-M4 models and training | research variants, ablation, training artifacts, and production TFLite export | ML teammate scope | docs/model_prd.md | ML-owned |
| Domain adaptation | zero-shot vs adapted real-data eval | ML teammate scope | docs/model_prd.md | ML-owned |
| Live inference integration | canonical `tflite` live mode, replay paths, and logging | src/infer/live_infer.py, src/infer/offline_replay.py, src/runtime/pipeline.py | tests/test_e2e_pipeline.py | Implemented |
